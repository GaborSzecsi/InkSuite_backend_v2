...
def _find_offset_with_context(
    full_text: str,
    raw_offset: int,
    left_ctx: Optional[str],
    right_ctx: Optional[str],
) -> int:
...
async def get_template(tid: str, request: Request):
    ...
    want = f"{(PUBLIC_BASE_URL or str(request.base_url)).rstrip('/')}/api/contracts/templates/{tid}/file"
...
async def get_template_file(tid: str):
    ...
    media_type = (
        mimetypes.guess_type(it.filename)[0]
...
async def insert_token(tid: str, body: InsertTokenPayload):
    ...
    try:
        ...
        para_text = p.text or ""
        effective_offset = _find_offset_with_context(
            para_text,
            body.char_offset,
            body.left_ctx,
            body.right_ctx,
...
async def save_template_mapping(tid: str, payload: TemplateMapping):
    ...
    put_bytes(
        _mapping_key(tid),
        json.dumps(mapping_obj, indent=2, ensure_ascii=False).encode("utf-8"),
